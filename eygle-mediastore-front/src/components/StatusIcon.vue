...
defineProps<{ media?: MediaDto; group?: MediaGroupDto; spaced?: boolean }>()
</script>
...
<template>
  <div>
    ...
    <v-tooltip v-if="media?.toTag || group?.toTag" text="To tag">
      <template #activator="{ props }">
    ...
    <v-tooltip v-if="media?.isPotentialBest" text="Potential best">
      <template #activator="{ props }">
        <v-icon icon="mdi-star-cog" color="orange-lighten-2" :class="spaced ? 'ml-4' : 'ml-2'" v-bind="props" />
      </template>
    </v-tooltip>
